...
async def test_create_event(client: AsyncClient):
    ...
    # Create valid event
    payload = {"steps": 1000}
    response = await client.post("/events", json={"user_id": 1, "module_id": module_id, "payload": payload})
    assert response.status_code == 201
    data = response.json()
    ...
    response = await client.post("/events", json={"user_id": 1, "module_id": module_id, "payload": bad_payload})
    assert response.status_code == 400
...
async def test_list_events(client: AsyncClient):
    ...
    # Create event
    await client.post("/events", json={"user_id": 99, "module_id": module_id, "payload": {}})
    ...
    # Filter
    response = await client.get(f"/events?user_id=99")
    assert response.status_code == 200
    assert len(response.json()) >= 1
    assert response.json()[0]["user_id"] == 99
